...
export class GoSettingsTab extends PluginSettingTab {
	...
	display(): void {
		...
		containerEl.createEl('h2', { text: 'GoBoard Settings' });
			containerEl.createEl('p', { text: 'Settings UI temporarily disabled due to recent refactor.' });

			/*
			// Temporarily disabled settings UI (broken due to missing plugin.settings API)
			// Размер доски
